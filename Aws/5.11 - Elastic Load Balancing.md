...
- Um listener verifica as solicitações de conexão de clientes usando o protocolo e a porta que você configurar

- As regras que você define para um ouvinte determinam como o balanceador de carga roteia solicitações para seus destinos registrados
...
- **_No balanceador de aplicação_**
  ...
  - Cada ouvinte verifica as solicitações de conexão de clientes usando o protocolo e a porta que você configurar
  ...
  - Você deve definir uma regra padrão para cada listener e pode adicionar regras que especificam diferentes grupos de destino com base no conteúdo da solicitação

  - Essa configuração também é conhecida como roteamento baseado em conteúdo

  - Cada grupo de destino roteia solicitações a um ou mais destinos registrados, como instâncias do EC2, usando o protocolo e o número de porta que você especificar

  - Você pode registrar um destino com vários grupos de destino

  - Você pode configurar verificações de integridade em cada grupo de destino
