...
- **_Hibernação_**
  ...
  - Quando você hiberna uma instância, o sistema operacional convidado preserva os dados na memória, persistindo-os no disco
  ...
  - Como interromper e reiniciar uma instância, o endereço IPv4 público original é liberado e um novo é atribuído quando você o reiniciar

  - Da mesma forma, se a instância estiver hibernando, ela reterá qualquer endereço IP elástico associado

  - Você não será cobrado pelo uso de uma instância em hibernação quando ela estiver no estado interrompido

  - No entanto, você será cobrado pelo armazenamento em todos os volumes do EBS, incluindo o armazenamento do conteúdo da memória que persiste no disco
...
- Muitas situações na nuvem exigem a criação de um novo servidor do zero, incluindo:
  ...
  1. **_Escalonamento automático_**
     - Você pode ter soluções que devem ser capazes de implantar novas instâncias sem intervenção humana
  ...
  2. **_Economia de custos_**
     - Você pode decidir que não precisa manter uma instância agora. No entanto, talvez você precise da capacidade de recriá-lo em um curto espaço de tempo. Os casos de uso de processamento em lote geralmente estão nessa categoria
  3. **_Downgrade_**
     - talvez você queira fazer downgrade de uma instância de alguma forma, para economizar custos. Por exemplo, você pode fazer um downgrade de uma instância executada em hardware dedicado a você como único cliente para um hardware com locação compartilhada. Ou talvez você queira fazer downgrade do tamanho de uma instância, por exemplo, de t2.xlarge para t2.large
  ...
  5. **_Atualização_**
     ...
     - Outra situação que pode exigir a execução de novas instâncias quando você deseja atualizar a arquitetura do sistema operacional ou o tipo de imagem
     ...
     - Você também pode optar por desativar as atualizações automáticas e instalar atualizações manualmente, dessa forma, você pode testar atualizações recentes para garantir que elas não quebrem nenhum aplicativo existente que possa estar sendo executado no servidor
