...
- **_Cofre_**: um contêiner para armazenar arquivos. Ao criar um cofre, você especifica o nome do cofre e a Região em que deseja que o cofre esteja localizado
...
- **_Política de acesso ao cofre_**
  ...
  - Você também pode usar uma **_política de bloqueio de cofre_** para ajudar a garantir que um cofre não possa ser alterado
...
- **_Trabalho_**
  ...
  - Ao executar uma consulta em um arquivo, você inicia um trabalho que fornece uma consulta de linguagem de consulta estruturada (SQL) e uma lista de objetos de arquivamento do Amazon S3 Glacier

# **Acesso**

- Ao arquivar dados com o Amazon S3 Glacier, você pode armazenar seus dados a um custo baixo (mesmo em comparação com o Amazon S3). No entanto, você não pode recuperar seus dados imediatamente quando quiser
...
- Para armazenar e acessar dados no Amazon S3 Glacier, você pode usar o Console de Gerenciamento da AWS. No entanto, apenas algumas operações — como criar ou excluir cofres e criar ou gerenciar políticas de arquivamento — estão disponíveis no console

- Quase todas as outras operações exigem que você use a interface de programação de aplicativo (API) REST do Amazon S3 Glacier ou os Kit de Desenvolvimento de Software (SDK) Java ou.NET AWS

- O uso dessas ferramentas permite que você interaja com o Amazon S3 Glacier por meio da AWS Command Line Interface (AWS CLI)

- Você também pode arquivar dados no Amazon S3 Glacier usando políticas de ciclo de vida
...
- **_Acelerado_**
  ...
  - As recuperações aceleradas permitem o acesso aos seus dados rapidamente quando você recebe uma solicitação urgente ocasional de um subconjunto de arquivos
  ...
  - As solicitações de capacidade provisionada estarão disponíveis garantidamente quando você precisar delas

- **_Padrão_**

  - As recuperações padrão permitem que você acesse qualquer um dos seus arquivos em várias horas
...
- **_Em massa_**
  ...
  - Você pode usá-las para recuperar grandes quantidades de dados — até mesmo petabytes — de forma econômica em um dia
...
- Você pode interagir com objetos no Amazon S3 Glacier de duas maneiras:
...
- Uma política de ciclo de vida do Amazon S3 permite que você mova um objeto para o Amazon S3 Glacier automaticamente

- Você faz isso especificando uma ação de transição para alterar sua classe de armazenamento para GLACIER
...
- Eles não aparecerão como um cofre ou um conjunto de arquivos se você usar a API do Amazon S3 Glacier com as credenciais da sua conta

- Os arquivos arquivados do Amazon S3 podem ser restaurados usando o console do Amazon S3 ou usando a API do Amazon S3

- Você também pode fazer upload, recuperar e gerenciar arquivos no Amazon S3 Glacier usando a API do Amazon S3 Glacier

- Os arquivos que você adicionou diretamente ao Amazon S3 Glacier com a API do Amazon S3 Glacier também podem ser recuperados por meio da API

- A recuperação de um arquivo do Amazon S3 Glacier é uma operação assíncrona em que você primeiro inicia um trabalho e, em seguida, baixa a saída após a conclusão do trabalho

- Para iniciar um trabalho de recuperação de arquivo, você usa a operação de API Initiate Job (POST jobs) ou o equivalente na AWS CLI ou nos Kit de Desenvolvimento de Software (SDK) da AWS
...
- Com o Glacier Select, você pode executar consultas e análises personalizadas nos seus dados armazenados no Amazon S3 Glacier sem restaurá-los para o Amazon S3

# **Politicas de ciclo de vida**

- Você deve automatizar o ciclo de vida dos dados armazenados no Amazon S3

- Usando políticas de ciclo de vida, você pode ter dados alternados em intervalos regulares entre diferentes tipos de armazenamento do Amazon S3

- Ele reduz os custos gerais porque você paga menos pelos dados quando eles se tornam menos importantes com o tempo
...
- Com as duas soluções, você pode transferir seus dados com segurança por HTTP seguro (HTTPS)
...
- Esse tipo de criptografia pode ser realizado de várias maneiras:
  ...
  3. Usando a criptografia do lado do servidor com chaves de criptografia fornecidas pelo cliente (SS-EC), você pode definir suas próprias chaves de criptografia. Com a chave de criptografia que você fornece como parte da sua solicitação, o Amazon S3 gerencia a criptografia, à medida que grava em discos, e a descriptografia quando você acessa seus objetos.

# **Segurança**

- Por padrão, apenas você pode acessar os seus dados

- Você pode habilitar e controlar o acesso aos seus dados no Amazon S3 Glacier usando o AWS Identity and Access Management (IAM)

- Você configura uma política do IAM que especifica o acesso do usuário
